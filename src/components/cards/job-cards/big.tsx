...
const ImageSlider = dynamic(() => import("@/components/image-slider"), {
  loading: () => <Skeleton className="overflow-hidden rounded-md border" />,
  ssr: false,
});
//todO: bid button is hard coded atm
const JobMap = dynamic(() => import("@/components/maps/job-map"), {
  loading: () => (
    <Skeleton className="h-[30vh] w-full rounded-md border md:h-full" />
  ),
  ssr: false,
});
    ...
    <Card {...props}>
      ...
      <ScrollArea {...scrollAreaProps}>
        ...
        <CardContent {...cardContentProps}>
          <div
            className={cn(
              "grid h-full gap-4 rounded-md md:w-[50%]",
          ...
          <div className="flex h-full w-full flex-col  rounded-md md:w-[50%]">
            {
              <JobMap
                className="h-[30vh] w-full rounded-md border md:h-full"
                zoom={zoom || 15}
                lng={lng}
                lat={lat}
              />
