import { mysqlTable, mysqlSchema, AnyMySqlColumn, primaryKey, varchar, int, text, timestamp, index, decimal, mysqlEnum, tinyint, smallint, unique } from "drizzle-orm/mysql-core"
...
export const companyIndustries = mysqlTable("company_industries", {
	companyId: varchar("company_id", { length: 50 }).notNull(),
	industryId: varchar("industry_id", { length: 50 }).notNull(),
},
(table) => {
	return {
		companyIndustriesCompanyIdIndustryId: primaryKey(table.companyId, table.industryId)
	}
});
...
export const companyProjects = mysqlTable("company_projects", {
	companyId: varchar("company_id", { length: 50 }).notNull(),
	projectId: varchar("project_id", { length: 50 }).notNull(),
},
(table) => {
	return {
		companyProjectsCompanyIdProjectId: primaryKey(table.companyId, table.projectId)
	}
});
...
(table) => {
	return {
		companyReviewsCompanyIdReviewId: primaryKey(table.companyId, table.reviewId)
	}
});
...
export const reviewMedia = mysqlTable("review_media", {
	reviewId: varchar("review_id", { length: 50 }).notNull(),
	mediaId: varchar("media_id", { length: 50 }).notNull(),
},
(table) => {
